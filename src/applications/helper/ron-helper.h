 ...
 */
class RonServerHelper
{
public:
  /**
   * Create RonServerHelper which will make life easier for people trying
   * to set up simulations with RONs.
   *
   * \param port The port the server will wait on for incoming packets
   */
  RonServerHelper (uint16_t port);
  ...
  /**
   * Create a RonServerApplication on the specified Node.
   *
  ...
  /**
   * Create a RonServerApplication on specified node
   *
   ...
   *
   * Create one RON server application on each of the Nodes in the
   * NodeContainer.
   *
...
public:
   ...
   * \param serverPort The port number of the remote RON server
   * \param overlayPort The port number of the RON overlay nodes
   */
  RonClientHelper (Ipv4Address ip, uint16_t serverPort, uint16_t overlayPort);
  ...
  /**
   * Given a pointer to a RonClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be RonClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the RonClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the RonClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
