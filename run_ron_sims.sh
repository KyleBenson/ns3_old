...
do
	...
	do
		...
		do
		    ...
		    ./waf --run "ron --file=rocketfuel/maps/$AS.cch --disaster=$disaster --RngSeed=$rseed --RngRun=$i --fail_prob=$pfail --latencies=rocketfuel/weights/$AS/latencies.intra --trace_file=$out_dir/run$i.out --contact_attempts=20 --timeout=0.5 --use_local_overlays=$local_overlays`if [ "$1" == "test" ]; then echo ' --verbose=1'; fi`"
